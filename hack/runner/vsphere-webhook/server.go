...
const (
	...
	workflowJobSetupRunner    string = "Start self-hosted VMC vSphere runner"
	workflowJobTeardownRunner string = "Stop self-hosted VMC vSphere runner"
...
func createOnlineRunner(g *GitHub, uniqueID string) error {
	klog.Infof("Check to see if %s runner exists...\n", uniqueID)
	client := GetGovmomiClient()
	...
	vmName, err := CreateVMGithubRunner(client, uniqueID, token)
	if err != nil {
		klog.Errorf("create VMC vSphere Runner failed. Err: %v\n", err)
		return err
	}
...
func createRunner(uniqueID string) error {
	...
	for i := 0; i < defaultNumOfTimesToRetry; i++ {
		err = createOnlineRunner(ghClient, uniqueID)
		if err == nil {
...
func deleteRunner(uniqueID string) error {
	...
	err = DeleteVM(client, uniqueID)
	if err != nil {
		klog.Errorf("deleteVmName failed. Err: %v\n", err)
		return err
	}
	...
	for {
		...
		for _, runner := range runners.Runners {
			...
			if strings.EqualFold(runnerStatus, runnerOnline) && !(*runner.Busy) {
				...
				err = DeleteVM(client, runnerName)
				if err != nil {
